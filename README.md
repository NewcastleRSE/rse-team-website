...
2. Copy an existing markdown file from the `_team/current` directory and rename it to your name in the format `firstname-surname.md`.
...
_Note: Do not edit the files in the `_site` directory, as this is generated by Jekyll. Instead, make changes to the source files in the `_team/current` directory._

## Editing

To edit the details on your staff page, simply update `_team/current/your-name.md` and commit. After a few minutes, the website will update automatically. Photos can also be updated. Ensure that either you overwrite the old file by using the same name, or update the markdown file to point to the new image.
